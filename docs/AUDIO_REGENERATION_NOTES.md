...
This will generate **19 voice samples** (TTS-generated):
- 3 greetings
- 9 dialogue samples (3 per relationship stage: hostile, curious, cooperative)
- 3 companion messages  
- 4 notifications

**Note**: The repository contains a total of 34 audio files, but only 19 are voice samples generated by TTS. The remaining 15 files are pre-existing sound effects (button clicks, menu sounds, etc.) located in `src/main/resources/audio/sounds/` that are not regenerated by this script.
...
```bash
# Check voice files were created (should be 19)
find src/main/resources/audio/{greetings,dialogue,notifications} -name "*.wav" -type f | wc -l

# Check all audio files including sound effects (should be 34 total)
find src/main/resources/audio -name "*.wav" -type f | wc -l

# Check file sizes (should be 50KB-400KB each for voice files)
ls -lh src/main/resources/audio/greetings/*.wav
ls -lh src/main/resources/audio/dialogue/*.wav
ls -lh src/main/resources/audio/notifications/*.wav

# Validate audio format (requires sox)
for f in src/main/resources/audio/{greetings,dialogue,notifications}/*.wav; do
